...
int Hough_Transform(SDL_Surface *image_surface)
{
    ...
    for (size_t x = 0; x < width; x++)
    {
        for (size_t y = 0; y < height; y++)
        {
            ...
            if (r == 255)
            {
                for (int teta = 0; teta < TETA; teta++)
                {
                    rho = x * cos(teta / 180 * M_PI) + y * sin(teta / 180 * M_PI);
                    f[(int)(rho * TETA + teta)] += 1;
//                    printf("%lf\n", f[(int)(rho * TETA + teta)]);
                }
            }
        }
    ...
    for (int r = 0; r < RHO; r++)
    {
        ...
        {
            if ((int)f[r * TETA + t] > t_max)
            {
                acc_max = (int)f[r * TETA + t];
                teta_max = t;
            }
        }
    }
